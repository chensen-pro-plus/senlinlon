...
export const UninstallCommand = {
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Senlinlon")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g senlinlon",
      pnpm: "pnpm uninstall -g senlinlon",
      bun: "bun remove -g senlinlon",
      yarn: "yarn global remove senlinlon",
      brew: "brew uninstall senlinlon",
      choco: "choco uninstall senlinlon",
      scoop: "scoop uninstall senlinlon",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall senlinlon -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Senlinlon!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# opencode") || content.includes(".senlinlon/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".senlinlon/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".senlinlon/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".senlinlon"))
    ) {
      continue
    }
