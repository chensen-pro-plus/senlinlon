...
const CLI_INSTALL_DIR: &str = ".senlinlon/bin";
const CLI_BINARY_NAME: &str = "senlinlon";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("senlinlon-install.sh");
...
pub fn create_command(app: &tauri::AppHandle, args: &str) -> Command {
    ...
    return app
        .shell()
        .sidecar("senlinlon-cli")
        .unwrap()
        .args(args.split_whitespace())
        .env("SENLINLON_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("SENLINLON_CLIENT", "desktop")
